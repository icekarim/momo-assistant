...
description: "A Claude Sonnet 4.6-powered agent loop with tool calling across 13 tools"
---

Momo uses a **Claude Sonnet 4.6-powered agent loop with tool calling**. When you send a message, Claude autonomously decides which tools to invoke — Gmail, Calendar, Tasks, Knowledge Graph, Granola, Jira — executes them, observes the results, and iterates (up to 6 rounds) until it has enough data to respond. Each tool has its own timeout (10-15s) and runs in a thread pool.
