...
## When a connector is unauthenticated

If a tool call hits an expired token or revoked scope, the agent no longer treats the empty response as "no data". The tool returns a structured error and Momo replies with the affected service name and a prompt to reconnect — for example:

> Jira credentials look expired, so I can't see your tickets right now. Reconnect Jira and ask me again.

This prevents Momo from confidently reporting "no results" when the underlying integration is actually broken.
