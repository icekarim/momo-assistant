...
## When a connector needs re-auth

If a source (Gmail, Calendar, Tasks, Granola, or Jira) can't authenticate when the briefing runs, Momo prepends a visible line to the briefing instead of quietly leaving that section empty:

```
⚠️ Jira connection needs re-auth
```

That way you can see at a glance that the missing content is a credential problem, not a genuinely quiet day. Reconnect the flagged service to restore that section on the next run.
