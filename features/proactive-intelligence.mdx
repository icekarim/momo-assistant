...
Prep context is scoped to the attendees and topic, not you. Momo excludes entries tied only to the owner identity (`OWNER_NAME` and email aliases) so a brand-new attendee doesn't get padded with your unrelated history. Retrieved entries older than `MEETING_PREP_CONTEXT_MAX_AGE_DAYS` (default 90) are dropped, and the merged person, project, and semantic results are reranked against the meeting title and non-owner attendees before being attached. If an attendee has no matching context, the prep says so explicitly instead of substituting stale material.
...
**3. Pattern detection** — Analyzes the last 30 days of knowledge graph entries to spot recurring people, projects, and tags. Feeds the patterns into Claude Haiku 4.5 for interpretation — surfaces things like recurring topics that need a dedicated discussion or frequent collaborators worth noting.
...
<Note>
Tuning knobs: `PROACTIVE_INTELLIGENCE_ENABLED`, `MEETING_PREP_ENABLED`, `MEETING_PREP_LOOKAHEAD_HOURS`, `MEETING_PREP_CONTEXT_MAX_AGE_DAYS`, `COMMITMENT_FOLLOWUP_DAYS`, `DRIFT_THRESHOLD_DAYS`, and `NUDGE_COOLDOWN_DAYS`. See the [environment variable reference](/setup/configuration#environment-variable-reference).
</Note>
