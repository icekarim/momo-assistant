...
Load conversation history from Firestore (up to 50 turns)
    │
    ▼
Agent loop (agent.py) with Claude and tool declarations:
    │
    ...
    │     │
    │     ▼
    │   Return results to Claude, which may call more tools
    │     │
...
├── gemini_service.py          # LLM service (Claude via claude_client): system prompt, chat/briefing/debrief generation
