...
| `/run-evals` | POST | Cloud Scheduler (daily) | Run LLM-as-judge evals against the golden dataset via Langfuse |
| `/connection-health` | POST | Cloud Scheduler (hourly) | Probe every external connector and alert Chat on auth-state transitions |
...
Cloud Scheduler ──► POST /run-evals                 (daily — nightly eval suite via Langfuse)
Cloud Scheduler ──► POST /connection-health        (hourly — connector auth health checks)
```
...
Failure surfacing: `/briefing` returns `500` when generation or Chat delivery fails so Cloud Scheduler records the failure and retries or alerts. If Claude returns an empty response, the generator retries once with double the token budget before raising. If generation still fails, Momo posts a best-effort ⚠️ notice to the Chat space so you know the briefing didn't arrive. Chat itself refuses to send empty messages, so a silent no-op delivery is no longer possible.
...
3. Batches up to 10 unseen emails → Claude Haiku 4.5 triage (returns JSON with `alert: true/false`, `priority`, `summary`)
...
3. Generates a short prep brief via Claude Haiku 4.5 with key context, open items, and talking points
...
3. Batch-extracts entities (decisions, commitments, blockers, etc.) via Claude Haiku 4.5
...
**`/run-evals`** — in `main.py`:

1. Runs the LLM-as-judge eval suite against the `momo-eval-golden` dataset (up to 50 items per run)
2. Exercises the live agent end-to-end and records results as a Langfuse experiment named `momo-eval-YYYYMMDD`
3. Returns `{"status": "ok", "experiment": "<name>"}` on success, or `{"status": "skipped"}` if the dataset is empty
4. Requires Langfuse keys to be configured (see [Configuration](/setup/configuration))

**`/connection-health`**:

1. Runs a lightweight authenticated probe against every configured connector in parallel (Jira, Google Workspace, Google Chat, Granola, each MCP server, Anthropic, Gemini, Firestore, Langfuse)
2. Classifies each result as `ok`, `auth_failed`, `unavailable`, or `skipped` (for disabled connectors)
3. Persists per-connector state in the `connection_health` Firestore collection
4. Posts a Chat alert only when a connector's status transitions (for example, `ok` → `auth_failed`, or recovery back to `ok`), plus a reminder at most every 12 hours while a connector is still failing
5. Returns `{ "status": "ok" | "degraded", "connectors": [...] }` so scheduler logs show a full snapshot

Run it hourly from Cloud Scheduler. Each connector's probe is capped at roughly 10 seconds so a single dead integration cannot stall the others.
