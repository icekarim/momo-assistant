...
class TestPersonScoring:
    ...
    def test_email_single_token(self):
        ...
        score = kr.score_person_pair("Craig", "craig@rokt.com")
        assert score == pytest.approx(0.95)
    ...
    def test_single_token_subset_is_queue_band(self):
        """Single token ⊂ multi-token name (Karim ⊂ Karim Tounkara) → QUEUE,
        not auto: confidence in [queue, auto)."""
        score = kr.score_person_pair("Karim", "Karim Tounkara")
...
class TestScorePairDispatcher:
    ...
    def test_judge_fn_can_override_queue_band(self):
        ...
        score = kr.score_pair(
            "Karim", "Karim Tounkara", "person", judge_fn=lambda ctx: 0.1
        )
...
class TestApplyPolicy:
    ...
    def test_queue_writes_pending(self):
        db = FakeDB()
        action = kr.apply_resolution(
            ("Karim", "Karim Tounkara"), "person", 0.80, db, _NOW
        )
        assert action == "queue"
        ...
        assert doc["pair"] == ["Karim", "Karim Tounkara"]
    ...
    def test_queue_threshold_boundary_is_queue(self):
        ...
        db = FakeDB()
        action = kr.apply_resolution(
            ("Karim", "Karim Tounkara"), "person", config.KG_MERGE_QUEUE_THRESHOLD, db, _NOW
        )
        assert action == "queue"
...
class TestDisplayNameSelection:
    ...
    def test_prefers_longer_full_name(self):
        name = kr._select_display_name(["Karim", "Karim Tounkara"], "person")
        assert name == "Karim Tounkara"
...
class TestRunResolution:
    ...
    def test_summary_counts(self):
        ...
        # Sarah↔email (auto), Karim↔Karim Tounkara (queue), Ads Team↔ads team (auto)
