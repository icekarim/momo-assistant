...
class TestRerunPreservesDecisions:
    # Entities that re-mine the ("Karim", "Karim Tounkara") queue-band pair.
    _ENTITIES = [{"related_people": ["Karim", "Karim Tounkara"],
                  "related_projects": []}]

    @pytest.mark.parametrize("decided_status", ["rejected", "approved"])
    def test_rerun_preserves_decided_status(self, decided_status):
        db = FakeDB()
        doc_id = _seed_queue(db, "Karim", "Karim Tounkara", decided_status)
    ...
    def test_rerun_still_queues_genuinely_pending(self):
        ...
        db = FakeDB()
        doc_id = _seed_queue(db, "Karim", "Karim Tounkara", "pending")
...
class TestMatchMergePairSpecificity:
    def _pending(self):
        return [
            {"pair": ["Karim", "Karim Tounkara"], "kind": "person",
             "confidence": 0.80, "status": "pending"},
            {"pair": ["Karim Tounkara", "karim@rokt.com"], "kind": "person",
             "confidence": 0.95, "status": "pending"},
        ]

    def test_most_specific_pair_wins_over_substring(self):
        match = agent._match_merge_pair(
            "approve Karim Tounkara and karim@rokt.com", self._pending()
        )
        assert match is not None
        assert match.get("pair") == ["Karim Tounkara", "karim@rokt.com"]

    def test_single_full_match_unaffected(self):
        match = agent._match_merge_pair("Karim and Karim Tounkara", self._pending())
        assert match is not None
        assert match.get("pair") == ["Karim", "Karim Tounkara"]
