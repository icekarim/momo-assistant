...
| `CLAUDE_MAX_TOKENS_BRIEFING` | No | `8192` | Max output tokens for the morning briefing. Reasoning models can spend the standard-tier budget on internal thinking and return no text. Keep this above the standard budget so the briefing always has room for output. |
| `CLAUDE_MAX_TOKENS_AGENT` | No | `8192` | Max output tokens for the interactive agent loop that replies in Google Chat. Same reasoning-burn risk as the briefing budget. If an agent turn returns no text, Momo retries once with double this budget before falling back to an in-voice apology. |
