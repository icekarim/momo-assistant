...
| --- | --- | --- | --- |
| **Core** |  |  |  |
| `ANTHROPIC_API_KEY` | Yes | — | Anthropic API key used by the Claude agent loop, briefings, debriefs, meeting prep, entity extraction, and email triage |
...
| **Claude Models** |  |  |  |
| `CLAUDE_MODEL_HAIKU` | No | `claude-haiku-4-5-20251001` | Model for light-tier tasks (email triage, entity extraction) |
| `CLAUDE_MODEL_SONNET` | No | `claude-sonnet-4-6` | Model for standard-tier tasks (agent loop, briefings, debriefs, prep) |
| `CLAUDE_MODEL_OPUS` | No | `claude-opus-4-8` | Model for deep-tier reasoning |
